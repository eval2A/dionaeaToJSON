...
if os.path.isfile(dionaeaSQLite):
	...
	for sqlTableIdentifier, sqlTableConfig in sqlTables.items():
		...
		totalEntries = totalEntries + int(currentIndex[sqlTableIdentifier])
		...
		totalNewEntries = totalNewEntries + int(newResults)
		...
		for result in results:
			...
			if sqlTableConfig['index'] == 'downloads':
				...
				if os.path.isfile(dionaeaBinariesPath + '/' + result['download_md5_hash']):
					...
					# Remove HTML files
					if 'removeHTMLFiles' in sqlTableConfig and sqlTableConfig['removeHTMLFiles'] == True:
				...
				if 'virusTotal' in sqlTableConfig and sqlTableConfig['virusTotal'] == True:
					result['virustotal'] = {}
					...
					if virusTotalsResult != None:
						...
						for virusTotalsScanResult in virusTotalsScanResults:
							...
							if virusTotalsScanResult['virustotalscan_result'] != None:
